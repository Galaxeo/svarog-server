...
# Set OpenAI API key (store it as an environment variable in PythonAnywhere)
...
def generate_text(prompt):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Give 1-2 active  "explain why, separated by the string |Ð|. Example: 'I|Ð|Because 2+2 does not equal 4.'",
            },
...
@app.route("/check_answer", methon and answer, determine if the answer is correct, incorrect, or half correct. "
                           "If the answer is correct, respond with 'C|Ð|Correct!'. If the answer is incorrect, respond with 'I'. "
                           "If the answer is half correct, respond with 'H'. After giving the grade, if the answer is not correct, "
...
def generate_text_endpoint():
    ...
    if not prompt:
        return jsonify({"error": "Prompt isrecall questions to user based on topics they studied or notes inputted. "
                           "Do not number the questions, separate the questions by the string |Ð|, and return all of "
                           "the questions on one line. For example: 'What did you learn about a random subject?|Ð|What "
                           "are the reasonings behind blank?'",
            },
